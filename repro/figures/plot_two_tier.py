...
def main():
    ...
    assert base["prompts"] == tr["prompts"] == cv["prompts"], "prompt sets differ"
    for name, blob in (("base", base), ("converged", cv), ("trained", tr)):
        assert "bd" in blob["seq"][0], f"no breakdown in the {name} file -- re-run the eval"
    ks = list(range(1, min(base["n"], cv["n"], tr["n"]) + 1))
    ...
    series = [("undertrained base · independent samples", base, "iid", "0.68", 0),
              ...
              (f"offline model, best checkpoint ({cv_step:,} steps) · independent samples",
               cv, "iid", "#2e7d32", 4),
              ("in-context model · independent samples", tr, "iid", "#3a6ea5", 2),
